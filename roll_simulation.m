...
SIM.T_step = 0.1;
SIM.Tsim = 78;  %Sim time in seconds
...
for t = DATA.T_vec
    ...
    if t>50
        if ~SIM.steered
            CONTROL.input_vec = CONTROL.input_vec + [0.01, 0.001];
%             SIM.steered = true;
        else
    ...
    CONTROL.input_vec(2) = max(-0.3,min(CONTROL.input_vec(2),0.3));
    if CONTROL.input_vec(1) > 20
        stop = true;
    end
    dynamics = @(t,X)(vehicle_dynamics(X(1),X(2),X(3),X(4),X(5),X(6),X(9),CONTROL.input_vec(1),CONTROL.input_vec(2)));
